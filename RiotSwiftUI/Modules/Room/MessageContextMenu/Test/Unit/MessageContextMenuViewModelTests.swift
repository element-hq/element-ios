...
@available(iOS 14.0, *)
class MessageContextMenuViewModelTests: XCTestCase {
    private enum Constants {
        static let presenceInitialValue: MessageContextMenuPresence = .offline
        static let displayName = "Alice"
    }
    var service: MockMessageContextMenuService!
    var viewModel: MessageContextMenuViewModelProtocol!
    var context: MessageContextMenuViewModelType.Context!
    var cancellables = Set<AnyCancellable>()
    override func setUpWithError() throws {
        service = MockMessageContextMenuService(displayName: Constants.displayName, presence: Constants.presenceInitialValue)
        viewModel = MessageContextMenuViewModel.makeMessageContextMenuViewModel(messageContextMenuService: service)
    ...
    func testPresenceUpdatesReceived() throws {
        ...
        let newPresenceValue1: MessageContextMenuPresence = .online
        let newPresenceValue2: MessageContextMenuPresence = .idle
