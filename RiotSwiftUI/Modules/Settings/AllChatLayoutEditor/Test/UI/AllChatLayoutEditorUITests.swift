...
class AllChatLayoutEditorUITests: MockScreenTest {
    ...
    func verifyAllChatLayoutEditorScreen() throws {
        guard let screenState = screenState as? MockAllChatLayoutEditorScreenState else { fatalError("no screen") }
        switch screenState {
        case .presence(let presence):
            verifyAllChatLayoutEditorPresence(presence: presence)
        case .longDisplayName(let name):
            verifyAllChatLayoutEditorLongName(name: name)
        }
    }

    func verifyAllChatLayoutEditorPresence(presence: AllChatLayoutEditorPresence) {
    ...
    func verifyAllChatLayoutEditorLongName(name: String) {
