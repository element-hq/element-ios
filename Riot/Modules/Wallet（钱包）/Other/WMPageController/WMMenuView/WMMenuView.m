...
#pragma mark - menuLineView

- (UIView *)menuLineView{
    if (!_menuLineView) {
        _menuLineView = [UIView new];
        _menuLineView.hidden = YES;
        _menuLineView.backgroundColor = UIColor.lightGrayColor;
    }
    return _menuLineView;
}

- (void)setShowMenuLine:(BOOL)showMenuLine{
    _showMenuLine = showMenuLine;
    self.menuLineView.hidden = !showMenuLine;
}

- (void)setMenuLineColor:(UIColor *)menuLineColor{
    _menuLineColor = menuLineColor;
    if (menuLineColor) {
        self.menuLineView.backgroundColor = menuLineColor;
    }
}
...
- (void)setCurrentIndex:(NSInteger)currentIndex{
    ...
    NSInteger tag = currentIndex + WMMENUITEM_TAG_OFFSET;
    NSInteger currentSelectedIndex = self.selItem.tag - WMMENUITEM_TAG_OFFSET;
    self.selectIndex = currentIndex;
    if (currentIndex == currentSelectedIndex || !self.selItem) { return; }
    
    WMMenuItem *item = (WMMenuItem *)[self viewWithTag:tag];
    [self.selItem setSelected:NO withAnimation:NO];
    self.selItem = item;
    [self.selItem setSelected:YES withAnimation:NO];
    [self.progressView setProgressWithOutAnimate:currentIndex];
    [self refreshContenOffset];
}
...
- (void)updateTitle:(NSString *)title atIndex:(NSInteger)index andWidth:(BOOL)update {
    ...
    PSTabMenuViewLabel *item = (PSTabMenuViewLabel *)[self viewWithTag:(WMMENUITEM_TAG_OFFSET + index)];
    item.text = title;
    if (!update) { return; }
    [self resetFrames];
}
...
- (void)layoutSubviews{
    ...
    self.menuLineView.frame = CGRectMake(0, self.frame.size.height-0.5, self.frame.size.width, 0.5);
}
...
- (void)addItems {
    ...
    for (int i = 0; i < self.titlesCount; i++) {
        ...
        WMMenuItem *item = nil;
        if ([self.dataSource respondsToSelector:@selector(menuView:menuItemForIndex:)]) {
            item = [self.dataSource menuView:self menuItemForIndex:i];
            item.frame = frame;
            item.delegate = self;
        }else{
            PSTabMenuViewLabel *defaultTabLabel = [[PSTabMenuViewLabel alloc]initWithFrame:frame];
            item = defaultTabLabel;
            item.delegate = self;
            defaultTabLabel.text = [self.dataSource menuView:self titleAtIndex:i];
            defaultTabLabel.textAlignment = NSTextAlignmentCenter;
            defaultTabLabel.normalSize    = [self sizeForState:WMMenuItemStateNormal atIndex:i];
            defaultTabLabel.selectedSize  = [self sizeForState:WMMenuItemStateSelected atIndex:i];
            defaultTabLabel.normalColor   = [self colorForState:WMMenuItemStateNormal atIndex:i];
            defaultTabLabel.selectedColor = [self colorForState:WMMenuItemStateSelected atIndex:i];
            defaultTabLabel.speedFactor   = self.speedFactor;
            if (self.fontName) {
                defaultTabLabel.font = [UIFont fontWithName:self.fontName size:defaultTabLabel.selectedSize];
            } else {
                defaultTabLabel.font = [UIFont systemFontOfSize:defaultTabLabel.selectedSize];
            }
        }
