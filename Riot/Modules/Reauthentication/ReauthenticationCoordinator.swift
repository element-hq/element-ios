...
final class ReauthenticationCoordinator: ReauthenticationCoordinatorType {
    ...
    init(parameters: ReauthenticationCoordinatorParameters) {
        ...
        userInteractiveAuthenticationService = UserInteractiveAuthenticationService(session: parameters.session)
        authenticationParametersBuilder = AuthenticationParametersBuilder()
        uiaViewControllerFactory = UserInteractiveAuthenticationViewControllerFactory()
    }
    
    // MARK: - Public methods
    
    func start() {
        if let authenticatedEndpointRequest = parameters.authenticatedEndpointRequest {
            start(with: authenticatedEndpointRequest)
        } else if let authenticationSession = parameters.authenticationSession {
            start(with: authenticationSession)
        } else {
    ...
    private func start(with authenticatedEndpointRequest: AuthenticatedEndpointRequest) {
        userInteractiveAuthenticationService.authenticatedEndpointStatus(for: authenticatedEndpointRequest) { result in
    ...
    private func start(with authenticationSession: MXAuthenticationSession) {
        if userInteractiveAuthenticationService.hasPasswordFlow(inFlows: authenticationSession.flows) {
            showPasswordAuthentication(with: authenticationSession)
        } else if let authenticationFallbackURL = userInteractiveAuthenticationService.firstUncompletedStageAuthenticationFallbackURL(for: authenticationSession) {
            showFallbackAuthentication(with: authenticationFallbackURL, authenticationSession: authenticationSession)
        } else {
            delegate?.reauthenticationCoordinator(self, didFailWithError: UserInteractiveAuthenticationServiceError.flowNotSupported)
        }
    }
    ...
    private func showPasswordAuthentication(with authenticationSession: MXAuthenticationSession) {
        ...
        let passwordViewController = uiaViewControllerFactory.createPasswordViewController(title: parameters.title, message: parameters.message) { [weak self] password in
        ...
        presentingViewController.present(passwordViewController, animated: true)
    }
    ...
    private func showFallbackAuthentication(with authenticationURL: URL, authenticationSession: MXAuthenticationSession) {
        ...
        let reauthFallbackViewController = ReauthFallBackViewController(url: authenticationURL.absoluteString)
        reauthFallbackViewController.title = parameters.title
