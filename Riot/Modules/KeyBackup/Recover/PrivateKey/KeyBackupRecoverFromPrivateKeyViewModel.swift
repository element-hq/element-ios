...
final class KeyBackupRecoverFromPrivateKeyViewModel: KeyBackupRecoverFromPrivateKeyViewModelType {
    ...
    func process(viewAction: KeyBackupRecoverFromPrivateKeyViewAction) {
        ...
        case .cancel:
            coordinatorDelegate?.keyBackupRecoverFromPrivateKeyViewModelDidCancel(self)
        }
    }
    ...
    private func recoverWithPrivateKey() {
        ...
        currentHTTPOperation = keyBackup.restore(usingPrivateKeyKeyBackup: keyBackupVersion, room: nil, session: nil, success: { [weak self] _, _ in
    ...
    private func update(viewState: KeyBackupRecoverFromPrivateKeyViewState) {
        viewDelegate?.keyBackupRecoverFromPrivateKeyViewModel(self, didUpdateViewState: viewState)
    }
}
