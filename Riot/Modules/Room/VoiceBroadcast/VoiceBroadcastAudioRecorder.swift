...
protocol VoiceBroadcastAudioRecorderDelegate: AnyObject {
    ...
    func audioRecorder(_ audioRecorder: VoiceBroadcastAudioRecorder, didFailWithError: Error)
}

enum VoiceBroadcastAudioRecorderError: Error {
    case genericError
}

class VoiceBroadcastAudioRecorder: NSObject, AVAudioRecorderDelegate {
    ...
    func recordWithOutputURL(_ url: URL) {
        ...
        do {
            ...
            delegateContainer.notifyDelegatesWithBlock { delegate in
                (delegate as? VoiceBroadcastAudioRecorderDelegate)?.audioRecorderDidStartRecording(self)
            }
        } catch {
            delegateContainer.notifyDelegatesWithBlock { delegate in
                (delegate as? VoiceBroadcastAudioRecorderDelegate)?.audioRecorder(self, didFailWithError: VoiceBroadcastAudioRecorderError.genericError)
            }
        }
    }
    ...
    func registerDelegate(_ delegate: VoiceBroadcastAudioRecorderDelegate) {
        delegateContainer.registerDelegate(delegate)
    }
    
    func deregisterDelegate(_ delegate: VoiceBroadcastAudioRecorderDelegate) {
    ...
    func audioRecorderDidFinishRecording(_ recorder: AVAudioRecorder, successfully success: Bool) {
        if success {
            delegateContainer.notifyDelegatesWithBlock { delegate in
                (delegate as? VoiceBroadcastAudioRecorderDelegate)?.audioRecorderDidFinishRecording(self)
            }
        } else {
            delegateContainer.notifyDelegatesWithBlock { delegate in
                (delegate as? VoiceBroadcastAudioRecorderDelegate)?.audioRecorder(self, didFailWithError: VoiceBroadcastAudioRecorderError.genericError)
...
extension VoiceBroadcastAudioRecorderDelegate {
    func audioRecorderDidStartRecording(_ audioRecorder: VoiceBroadcastAudioRecorder) {}
    func audioRecorderDidPauseRecording(_ audioRecorder: VoiceBroadcastAudioRecorder) {}
    func audioRecorderDidResumeRecording(_ audioRecorder: VoiceBroadcastAudioRecorder) {}
    func audioRecorderDidFinishRecording(_ audioRecorder: VoiceBroadcastAudioRecorder) {}
